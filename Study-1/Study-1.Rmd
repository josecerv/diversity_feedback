---
title: "Study 1 (Gender NPR Study)"
...
## Test for treatment × set interaction
interaction_model <- lm(women_proportion ~ treatment * set_num, data=d0)
int_summary <- robust_summary(interaction_model)

# Extract interaction and set main effect
int_coef <- int_summary$coefficients["treatment:set_num", "Estimate"]
int_p <- int_summary$coefficients["treatment:set_num", "Pr(>|t|)"]
set_coef <- int_summary$coefficients["set_num", "Estimate"]
set_p <- int_summary$coefficients["set_num", "Pr(>|t|)"]

cat(sprintf("Treatment × Set interaction: B = %.3f, p = %.3f\n", int_coef, int_p))
cat(sprintf("Main effect of stimulus set: B = %.3f, p = %.3f\n", set_coef, set_p))
...
## Model 2: Which feedback was shown with women, remove constant due to collinearity
...
## Model 3: Robust to demographic controls
r3 <- lm(women_proportion ~ women_feedback + gender_code + race_code + age, data=d0)
...
p_combined <- ggplot(df_combined, aes(x = Condition, y = freq*100, fill = Condition)) +
  ...
  geom_text(aes(label=paste0(sprintf("%.1f", freq*100),"%")),
            position=position_dodge(width=0.7), vjust=5, size = 5, color = "white", family = plot_font) +
  ...
  facet_wrap(~Category, nrow = 1, strip.position = "bottom") +
   ...
   geom_text(data = df_combined %>% filter(Condition == "\"Treatment\""),
             ...
             inherit.aes = FALSE, vjust = 0, size = 5, family = plot_font) +
  theme_bw() +
  ...
  theme(plot.caption = element_text(face = "italic", family = plot_font),
        legend.position = c(0.5, 0.95),
        legend.title = element_blank(),
        legend.direction = "horizontal",
        legend.text = element_text(size = 20, family = plot_font),
        ...
        axis.title.x = element_text(face="bold", size = 21, vjust = 22, family = plot_font),
        plot.title = element_blank(),
        axis.title.y = element_text(size = 20, color = "black", family = plot_font),
        axis.text.x = element_blank(),
        axis.ticks = element_blank(),
        axis.text.y = element_text(size = 20, color = "black", family = plot_font),
        strip.text = element_text(size = 20, color = "black", family = plot_font),
        strip.background = element_rect(colour = "white", fill = "white"),
        text = element_text(family = plot_font))

#print(p_combined)

# Save the plot (uses plot_font which defaults to Times New Roman if available)
# PDF export using cairo_pdf for proper font embedding
# ggsave("Figure-Study1.pdf", plot = p_combined, width = 10, height = 9, units = "in", device = cairo_pdf, family = plot_font)

# # PNG export - use type="cairo" for Windows to properly render fonts
# ggsave("Figure-Study1.png", plot = p_combined, width = 10, height = 9, units = "in", dpi = 600, type = "cairo")

# Save RDS for combined figure
saveRDS(p_combined, file = "p_combined_study1.rds")
```
