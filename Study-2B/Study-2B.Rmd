...
if(USE_API) {
  ...
  qual_data <- fetch_survey(surveyID='SV_cGCFBxyTGbzsZvw',  # New Study ID
...
minority <- c(
  'Alice Walker', 'Gabriel Garcia Marquez', 'Isabel Allende',
  'Sandra Cisneros', 'Toni Morrison', 'WEB Du Bois'
)

multi_genre <- c(
  'Agatha Christie', 'Emily Bronte', 'Gabriel Garcia Marquez', 'George Orwell',
  'Herman Melville', 'Isabel Allende', 'Joyce Carol Oates', 'J.R.R. Tolkien',
  'Louisa May Alcott', 'Lucy Maud', 'Ray Bradbury', 'Sandra Cisneros',
  'Toni Morrison', 'WEB Du Bois'
)
...
classic_50plus <- c(
  'Agatha Christie', 'Charles Dickens', 'Emily Bronte', 'Ernest Hemingway',
  'F. Scott Fitzgerald', 'Gabriel Garcia Marquez', 'George Orwell', 'Herman Melville',
  'J.D. Salinger', 'J.R.R. Tolkien', 'Jack London', 'Jane Austen', 'John Steinbeck',
  'Louisa May Alcott', 'Lucy Maud', 'Nathaniel Hawthorne', 'Ray Bradbury',
  'Sylvia Plath', 'WEB Du Bois'
)
...
if(USE_API) {
  ...
  d0 <- qual_data |>
    ...
    mutate(
      # Minority feedback detection
      minority_feedback = as.numeric(grepl("were racial minorities", feedbackItem1) |
                    grepl("were racial minorities", feedbackItem2) |
                    grepl("were racial minorities", feedbackItem3)),
      ...
      minority_pick = case_when(`choice-7` %in% minority ~ 1, TRUE ~ 0),
      ...
      base_minority = rowSums(across(`choice-1`:`choice-6`, ~ . %in% minority))
    ) |>
    mutate(
      across(c(I1:E3),
             ~ case_when(
    ...
    dplyr::select(minority_feedback, forms_shown, sold30m_shown, classic_shown,
                  minority_pick, forms_pick, sold30m_pick, classic_pick, base_minority, `choice-1`:`choice-7`,
...
\hline
\texttt{minority\_feedback} & Binary indicator of whether a participant was randomly assigned to racial minority feedback condition. \\
\hline
\texttt{minority\_pick} & Binary indicator of whether a participant selected a racial minority author for their seventh selection \\
\hline
...
\hline
\texttt{base\_minority} & Count of the number of racial minority authors selected in the initial six authors. \\
\hline
...
cat('Mean (num of initial racial minorities selected): ', round(mean(d0$base_minority), 2))
cat('SD (num of initial racial minorities selected): ',round(sd(d0$base_minority), 2))

## Initial Selections (%)

cat('Percentage (initial racial minorities selected): ', round(mean(d0$base_minority), 2)/6)
cat('SD (initial racial minorities selected): ',round(sd(d0$base_minority), 2)/6)

d0 |>
  dplyr::select(base_minority, minority_feedback) |>
  dplyr::group_by(minority_feedback) |>
  dplyr::summarize(mean = mean(base_minority/6))

t.test(base_minority/6 ~ minority_feedback, data=d0)
...
# primary model
r1 <- lm(minority_pick ~ minority_feedback, data=d0)
...
## which feedback was shown with minority, remove constant due to collinearity
r2 <- lm(minority_pick ~ minority_feedback + classic_shown + sold30m_shown + forms_shown - 1, data=d0)
...
r3 <- lm(minority_pick ~ minority_feedback + gender_code + race_code + age, data=d0)
...
r4 <- glm(minority_pick ~ minority_feedback, family = binomial, data=d0)
summary(r4)
# Odds ratio
...
```{r echo=TRUE, results='markup'}
## interaction of base minority
# primary model
r_interaction <- lm(minority_pick ~ minority_feedback*base_minority, data=d0)
...
r_poli_moderation <- lm(minority_pick ~ minority_feedback * poli_centered, data=d0)
...
liberal_slope <- coef(r_poli_moderation)["minority_feedback"] +
                 coef(r_poli_moderation)["minority_feedback:poli_centered"] * (-2)

# Moderate (at mean, centered = 0)
moderate_slope <- coef(r_poli_moderation)["minority_feedback"]

# Conservative (1 SD above mean, approximately conservative)
conservative_slope <- coef(r_poli_moderation)["minority_feedback"] +
                      coef(r_poli_moderation)["minority_feedback:poli_centered"] * (2)

cat("\n\nSimple Slopes Analysis:\n")
cat("Effect of minority feedback for liberals (ideology = 2): ", round(liberal_slope * 100, 2), "%\n")
cat("Effect of minority feedback for moderates (ideology = 4): ", round(moderate_slope * 100, 2), "%\n")
cat("Effect of minority feedback for conservatives (ideology = 6): ", round(conservative_slope * 100, 2), "%\n")

# Visualization of interaction
library(ggplot2)
# Create prediction data
pred_data <- expand.grid(
  minority_feedback = c(0, 1),
  poli_centered = seq(-3, 3, by = 0.5)  # From very liberal to very conservative
)
pred_data$minority_pick <- predict(r_poli_moderation, newdata = pred_data)
pred_data$poli_label <- pred_data$poli_centered + 4  # Convert back to 1-7 scale

# Plot interaction
p_interaction <- ggplot(pred_data, aes(x = poli_label, y = minority_pick * 100,
                                       color = factor(minority_feedback))) +
  geom_line(size = 1.2) +
  scale_color_manual(values = c("0" = "#990000", "1" = "#011F5B"),
                     labels = c("0" = "Control", "1" = "Minority Feedback")) +
  scale_x_continuous(breaks = 1:7,
                     labels = c("Extremely\nLiberal", "Liberal", "Slightly\nLiberal",
                               "Moderate", "Slightly\nConservative", "Conservative",
                               "Extremely\nConservative")) +
  labs(x = "Political Ideology",
       y = "% Selecting Racial Minority Author",
...
r_party_moderation <- lm(minority_pick ~ minority_feedback * party_democrat +
                                      minority_feedback * party_independent, data=d0)
...
republican_effect <- coef(r_party_moderation)["minority_feedback"]

# Effect for Democrats
democrat_effect <- coef(r_party_moderation)["minority_feedback"] +
                   coef(r_party_moderation)["minority_feedback:party_democrat"]

# Effect for Independents
independent_effect <- coef(r_party_moderation)["minority_feedback"] +
                      coef(r_party_moderation)["minority_feedback:party_independent"]

cat("\n\nSimple Effects Analysis:\n")
cat("Effect of minority feedback for Republicans: ", round(republican_effect * 100, 2), "%\n")
cat("Effect of minority feedback for Democrats: ", round(democrat_effect * 100, 2), "%\n")
cat("Effect of minority feedback for Independents: ", round(independent_effect * 100, 2), "%\n")

# Compute means for each group and condition
party_means <- d0 |>
  group_by(poli_party, minority_feedback) |>
  summarize(
    mean_minority_pick = mean(minority_pick, na.rm = TRUE),
    n = n()
  ) |>
...
# Visualization of interaction
p_party_interaction <- ggplot(party_means, aes(x = poli_party, y = mean_minority_pick * 100,
                                                fill = factor(minority_feedback))) +
...
minority_est <- extract_regression_estimates(r1)
...
study2B_estimates <- data.frame(
    Study = "STUDY 2B",
    Stimuli = c("minority", "classic", "sold30m", "forms"),
    Estimate = c(minority_est$estimate, classic_est$estimate, sold30m_est$estimate, forms_est$estimate),
    SE = c(minority_est$se, classic_est$se, sold30m_est$se, forms_est$se),
    CI_Lower = c(minority_est$ci_lower, classic_est$ci_lower, sold30m_est$ci_lower, forms_est$ci_lower),
    CI_Upper = c(minority_est$ci_upper, classic_est$ci_upper, sold30m_est$ci_upper, forms_est$ci_upper)
)

# Save the estimates to an RDS file
saveRDS(study2B_estimates, file = "study2B_estimates.rds")
...
# Direct effect model
dir.fit.internal <- lm(minority_pick ~ minority_feedback, data=d0)

# Mediator model
med.fit.internal <- lm(internal ~ minority_feedback, data = d0)

# Outcome model including mediator
out.fit.internal <- lm(minority_pick ~ minority_feedback + internal, data = d0)

# Mediation analysis
med.out.internal <- mediate(med.fit.internal, out.fit.internal, boot = TRUE,
                            treat = "minority_feedback", boot.ci.type = "perc", mediator = "internal", sims = 10000)
...
# Mediator model
med.fit.external <- lm(external ~ minority_feedback, data = d0)

# Outcome model including mediator
out.fit.external <- lm(minority_pick ~ minority_feedback + external, data = d0)

# Mediation analysis
med.out.external <- mediate(med.fit.external, out.fit.external, boot = TRUE,
                            treat = "minority_feedback", boot.ci.type = "perc", mediator = "external", sims = 10000)
...
out.fit.combined <- lm(minority_pick ~ minority_feedback + internal + external, data = d0)
...
p_minority <- robust_summary(r1)$coefficients["minority_feedback", "Pr(>|t|)"]
...
sig_minority <- get_sig_stars(p_minority)
sig_sold30m <- get_sig_stars(p_sold30m)
sig_forms <- get_sig_stars(p_forms)
sig_classic <- get_sig_stars(p_classic)

dminority_plot <- d0 |>
  dplyr::select(minority_feedback, minority_pick) |>
  dplyr::group_by(minority_feedback) |>
  dplyr::summarise(
    n = n(),
    freq = mean(minority_pick),
    sd = sd(minority_pick) * 100,
    se = (sd(minority_pick) / sqrt(n())) * 100
  ) |>
  dplyr::mutate(
    minority_feedback = case_when(
      minority_feedback == 1 ~ "\"Treatment\"",
      TRUE ~ "\"Control\""
    )
  ) |>
  dplyr::rename(Condition = minority_feedback)
...
df_combined <- bind_rows(
  ...
  dminority_plot %>% mutate(Category = "\nWere Racial\nMinorities", sig_label = sig_minority)
, .id = "id") %>%
...
ggsave("Figure-S1.pdf", plot = p_combined, width = 10, height = 8, units = "in", device = cairo_pdf, family = "Times New Roman")
```
...
sold30m.eqn <- list(
  sold30meq = as.numeric(sold30m_pick) ~ sold30m_shown + forms_shown + classic_shown + minority_feedback - 1,
  minorityeq = as.numeric(minority_pick) ~ sold30m_shown + forms_shown + classic_shown + minority_feedback - 1
)

# Define restriction matrix
restrict_sold30m <- "sold30meq_sold30m_shown - minorityeq_minority_feedback = 0"
...
forms.eqn <- list(
  formseq = as.numeric(forms_pick) ~ sold30m_shown + forms_shown + classic_shown + minority_feedback - 1,
  minorityeq = as.numeric(minority_pick) ~ sold30m_shown + forms_shown + classic_shown + minority_feedback - 1
)

# Define restriction matrix
restrict_forms <- "formseq_forms_shown - minorityeq_minority_feedback = 0"
...
classic.eqn <- list(
  classiceq = as.numeric(classic_pick) ~ sold30m_shown + forms_shown + classic_shown + minority_feedback - 1,
  minorityeq = as.numeric(minority_pick) ~ sold30m_shown + forms_shown + classic_shown + minority_feedback - 1
)

# Define restriction matrix
restrict_classic <- "classiceq_classic_shown - minorityeq_minority_feedback = 0"
...
data.frame(
  `Test` = c("Minority Feedback - Sold30M Feedback", "Minority Feedback - Forms Feedback", "Minority Feedback - Classic Feedback"),
