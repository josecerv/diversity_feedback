...
datasets <- list(
  s2A = list(data = s2A, type = "race"),
  s2B = list(data = s2B, type = "gender"),
...
stargazer(models$s2A, models$s2B, models$s3A, models$s3B,
          type = "latex",
          column.labels = c("Study 2A", "Study 2B", "Study 3A", "Study 3B"),
          model.numbers = FALSE,
          se = list(robust_se$s2A, robust_se$s2B, robust_se$s3A, robust_se$s3B),
          keep.stat = c("n", "rsq"),
...
wald_results$Study <- c("Study 2A", "Study 2B", "Study 3A", "Study 3B")
...
robust_confint(models$s2A)
robust_confint(models$s2B)
