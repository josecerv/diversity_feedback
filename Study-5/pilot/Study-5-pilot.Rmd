...
if(USE_API) {
  ...
  qual_data <- fetch_survey(surveyID='SV_2f6hIXCPIwAylDw',
                     label = T,
                     convert = F,
  ...
  d0 <- read.csv('Study-5-pilot.csv', check.names = F)
...
if(USE_API) {
  ...
  write.csv(d0, 'Study-5-pilot.csv', row.names = FALSE, quote = TRUE)
}
```
...
# Save the plot
ggsave("Study-5-pilot-figure.pdf", plot = p_combined, width = 10, height = 8, units = "in", device = cairo_pdf)
```
...
logit_2x2 <- glm(female_pick ~ treatment * women_pool,
                 data = d0,
                 family = binomial(link = "logit"))
summary(logit_2x2)
...
cat("Model: female_pick ~ treatment * women_pool + man + white + age + missing indicators\n\n")

model_2x2_controls <- lm(female_pick ~ treatment * women_pool +
                          man + white + age_clean +
...
if(USE_API & exists("qual_data")) {
  ...
  if(total_dropouts > 0) {
    ...
    model_dropout_man <- lm(female_pick ~ treatment * women_pool, data = d0_dropout_man)
    cat("Model: female_pick ~ treatment * women_pool\n\n")
    ...
    model_dropout_woman <- lm(female_pick ~ treatment * women_pool, data = d0_dropout_woman)
    cat("Model: female_pick ~ treatment * women_pool\n\n")
