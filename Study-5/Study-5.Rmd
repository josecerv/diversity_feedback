---
title: "Study 5 (Gender 2x2 Business Panel)"
...
if(USE_API) {
  ...
  qual_data <- fetch_survey(surveyID='SV_06V9Nrg8gROA9JI',
                     label = T,
                     convert = F,
...
if(USE_API) {
  ...
  d0 <- qual_data |>
    ...
    slice(1:1200)  # Pre-registered sample size
...
# Create women_overrep indicator (opposite of men_pool)
# women_overrep = 1 means women overrepresented (75% women)
# women_overrep = 0 means women underrepresented (25% women)
...
# Predictors: (1) gender feedback condition, (2) women overrepresented pool, (3) interaction
# Note: We use women_overrep for interpretability (reference = underrep condition)
# This allows testing H1 via coefficient on (1) and H2 via coefficient on (3)

# Create binary variables for controls
d0 <- d0 %>%
  mutate(
    female_participant = ifelse(gender == "Woman", 1, 0),
    white_participant = ifelse(race == "White / Caucasian", 1, 0)
  )

cat("=== 2x2 Interaction: Treatment × Women Overrepresented Pool ===\n")
cat("(women_overrep: 1 = women overrepresented/75%, 0 = women underrepresented/25%)\n\n")

# Model 5: Without controls
cat("--- MODEL 5: OLS WITHOUT CONTROLS ---\n")
cat("Model: female_pick ~ treatment * women_overrep\n\n")
model_2x2 <- lm(female_pick ~ treatment * women_overrep, data = d0)
robust_summary(model_2x2)
robust_confint(model_2x2)

# Model 6: With demographic controls
cat("\n\n--- MODEL 6: OLS WITH DEMOGRAPHIC CONTROLS ---\n")
cat("Model: female_pick ~ treatment * women_overrep + female_participant + white_participant + age\n\n")

model_2x2_controls <- lm(female_pick ~ treatment * women_overrep + female_participant + white_participant + age,
                          data = d0)
...
cat("\n\n--- LOGISTIC REGRESSION ROBUSTNESS CHECK ---\n")
cat("Model: female_pick ~ treatment * women_overrep (logit)\n\n")

logit_2x2 <- glm(female_pick ~ treatment * women_overrep, data = d0, family = binomial)
summary(logit_2x2)

cat("\nLogit with controls:\n")
logit_2x2_controls <- glm(female_pick ~ treatment * women_overrep + female_participant + white_participant + age,
                          data = d0, family = binomial)
summary(logit_2x2_controls)
...
### Women Underrepresented Pool (25% Women)

```{r men-pool, echo=FALSE, results='markup'}
# Create binary variables for controls (used throughout)
d0_underrep <- d0 %>%
  filter(men_pool == 1) %>%
  mutate(
    female_participant = ifelse(gender == "Woman", 1, 0),
...
model_men <- lm(female_pick ~ treatment, data = d0_underrep)

cat("=== WOMEN UNDERREPRESENTED POOL (MEN POOL, 25% WOMEN) ===\n")
cat("--- MODEL 1: OLS WITHOUT CONTROLS ---\n")
cat("Model: female_pick ~ treatment\n\n")
robust_summary(model_men)
robust_confint(model_men)

# Model 2: With demographic controls
cat("\n\n--- MODEL 2: OLS WITH DEMOGRAPHIC CONTROLS ---\n")
cat("Model: female_pick ~ treatment + female_participant + white_participant + age\n\n")

model_men_controls <- lm(female_pick ~ treatment + female_participant + white_participant + age,
                         data = d0_underrep)
robust_summary(model_men_controls)
robust_confint(model_men_controls)

# Logistic regression robustness check
cat("\n\n--- LOGISTIC REGRESSION ROBUSTNESS CHECK ---\n")
cat("Model: female_pick ~ treatment (logit)\n\n")

logit_men <- glm(female_pick ~ treatment, data = d0_underrep, family = binomial)
summary(logit_men)

cat("\nLogit with controls:\n")
logit_men_controls <- glm(female_pick ~ treatment + female_participant + white_participant + age,
                          data = d0_underrep, family = binomial)
summary(logit_men_controls)
...
### Women Overrepresented Pool (75% Women)
...
# Model 3: Main effect model
cat("--- MODEL 3: OLS WITHOUT CONTROLS ---\n")
cat("Model: female_pick ~ treatment\n\n")
model_women <- lm(female_pick ~ treatment, data = d0_overrep)
robust_summary(model_women)
robust_confint(model_women)

# Model 4: With demographic controls
cat("\n\n--- MODEL 4: OLS WITH DEMOGRAPHIC CONTROLS ---\n")
cat("Model: female_pick ~ treatment + female_participant + white_participant + age\n\n")

model_women_controls <- lm(female_pick ~ treatment + female_participant + white_participant + age,
                           data = d0_overrep)
robust_summary(model_women_controls)
robust_confint(model_women_controls)

# Logistic regression robustness check
cat("\n\n--- LOGISTIC REGRESSION ROBUSTNESS CHECK ---\n")
cat("Model: female_pick ~ treatment (logit)\n\n")

logit_women <- glm(female_pick ~ treatment, data = d0_overrep, family = binomial)
summary(logit_women)

cat("\nLogit with controls:\n")
logit_women_controls <- glm(female_pick ~ treatment + female_participant + white_participant + age,
                            data = d0_overrep, family = binomial)
summary(logit_women_controls)
...
## Wald Test: Comparing Treatment Effect Magnitudes Across Pools

```{r wald-test, echo=FALSE, results='markup'}
cat("=== WALD TEST: EQUAL MAGNITUDE OF TREATMENT EFFECTS ===\n\n")
cat("Testing H0: B_underrep + B_overrep = 0\n")
cat("(i.e., treatment effects are equal in magnitude but opposite in direction)\n\n")
...
# Calculate SUM of treatment effects (tests equal magnitude with opposite signs)
# If effects are equal in magnitude but opposite in direction, sum should equal 0
sum_effects <- effect_men + effect_women

# Standard error of the sum (assuming independence)
se_sum <- sqrt(se_men^2 + se_women^2)

# Wald test statistic
wald_stat <- sum_effects / se_sum

# P-value (two-tailed)
p_value <- 2 * (1 - pnorm(abs(wald_stat)))

# 95% CI for the sum
ci_lower <- sum_effects - 1.96 * se_sum
ci_upper <- sum_effects + 1.96 * se_sum

# Print results
cat("Treatment Effect (Underrep Pool): ", sprintf("%.4f", effect_men),
    " (SE = ", sprintf("%.4f", se_men), ")\n", sep = "")
cat("Treatment Effect (Overrep Pool):  ", sprintf("%.4f", effect_women),
    " (SE = ", sprintf("%.4f", se_women), ")\n\n", sep = "")

cat("Sum of Treatment Effects:         ", sprintf("%.4f", sum_effects), "\n", sep = "")
cat("Standard Error of Sum:            ", sprintf("%.4f", se_sum), "\n", sep = "")
cat("Wald Statistic (z):               ", sprintf("%.4f", wald_stat), "\n", sep = "")
cat("P-value (two-tailed):             ", sprintf("%.4f", p_value), "\n", sep = "")
cat("95% CI for Sum:                   [", sprintf("%.4f", ci_lower), ", ",
    sprintf("%.4f", ci_upper), "]\n\n", sep = "")

cat("Interpretation: A non-significant p-value indicates the treatment effects\n")
cat("are approximately equal in magnitude (opposite in direction).\n")
```
...
## Mediation Analysis: Women Underrepresented Pool

```{r mediation-analysis-underrep, echo=TRUE, results='markup'}
# Set seed for reproducibility
set.seed(456)

# Filter to women underrepresented pool only (men pool, 25% women)
d_men_pool <- d0 |> filter(men_pool == 1, !is.na(fairness))

cat("=======================================================\n")
cat("MEDIATION ANALYSIS: WOMEN UNDERREPRESENTED POOL\n")
cat("=======================================================\n\n")

cat("Sample size for mediation analysis:", nrow(d_men_pool), "\n\n")
...
# Direct effect model
dir.fit.fairness.underrep <- lm(female_pick ~ treatment, data = d_men_pool)

# Mediator model (a path)
med.fit.fairness.underrep <- lm(fairness ~ treatment, data = d_men_pool)

# Outcome model including mediator (b path)
out.fit.fairness.underrep <- lm(female_pick ~ treatment + fairness, data = d_men_pool)

# Mediation analysis using Imai's mediation package
med.out.fairness.underrep <- mediate(med.fit.fairness.underrep, out.fit.fairness.underrep, boot = TRUE,
                            treat = "treatment", boot.ci.type = "perc", mediator = "fairness", sims = 10000)

# Sensitivity analysis
sens.out.fairness.underrep <- medsens(med.out.fairness.underrep, rho.by = 0.01, eps = .01, effect.type = "indirect", sims = 10000)

# Sobel test for fairness
sobel.fairness.underrep <- sobel_test(med.fit.fairness.underrep, out.fit.fairness.underrep, "fairness")

# Print and visualize results for fairness
cat("\n--- FAIRNESS MEDIATION ---\n\n")
cat("Sobel test for Fairness\n")
print(sobel.fairness.underrep)
summary(med.out.fairness.underrep)
plot(sens.out.fairness.underrep)
...
# Mediator model (a path)
med.fit.internal.underrep <- lm(internal_motivation ~ treatment, data = d_men_pool)

# Outcome model including mediator (b path)
out.fit.internal.underrep <- lm(female_pick ~ treatment + internal_motivation, data = d_men_pool)

# Mediation analysis
med.out.internal.underrep <- mediate(med.fit.internal.underrep, out.fit.internal.underrep, boot = TRUE,
                            treat = "treatment", boot.ci.type = "perc", mediator = "internal_motivation", sims = 10000)

# Sensitivity analysis
sens.out.internal.underrep <- medsens(med.out.internal.underrep, rho.by = 0.01, eps = .01, effect.type = "indirect", sims = 10000)

# Sobel test for internal motivation
sobel.internal.underrep <- sobel_test(med.fit.internal.underrep, out.fit.internal.underrep, "internal_motivation")

# Print and visualize results for internal motivation
cat("\n--- INTERNAL MOTIVATION MEDIATION ---\n\n")
cat("Sobel test for Internal Motivation\n")
print(sobel.internal.underrep)
summary(med.out.internal.underrep)
plot(sens.out.internal.underrep)
...
# Mediator model (a path)
med.fit.external.underrep <- lm(external_motivation ~ treatment, data = d_men_pool)

# Outcome model including mediator (b path)
out.fit.external.underrep <- lm(female_pick ~ treatment + external_motivation, data = d_men_pool)

# Mediation analysis
med.out.external.underrep <- mediate(med.fit.external.underrep, out.fit.external.underrep, boot = TRUE,
                            treat = "treatment", boot.ci.type = "perc", mediator = "external_motivation", sims = 10000)

# Sensitivity analysis
sens.out.external.underrep <- medsens(med.out.external.underrep, rho.by = 0.01, eps = .01, effect.type = "indirect", sims = 10000)

# Sobel test for external motivation
sobel.external.underrep <- sobel_test(med.fit.external.underrep, out.fit.external.underrep, "external_motivation")

# Print and visualize results for external motivation
cat("\n--- EXTERNAL MOTIVATION MEDIATION ---\n\n")
cat("Sobel test for External Motivation\n")
print(sobel.external.underrep)
summary(med.out.external.underrep)
plot(sens.out.external.underrep)
...
# Compute the correlation coefficient and p-value between mediators
cat("\n--- MEDIATOR CORRELATIONS ---\n\n")
cor_fair_int.underrep <- cor.test(d_men_pool$fairness, d_men_pool$internal_motivation)
cor_fair_ext.underrep <- cor.test(d_men_pool$fairness, d_men_pool$external_motivation)
cor_int_ext.underrep <- cor.test(d_men_pool$internal_motivation, d_men_pool$external_motivation)

cat("Correlation (Fairness, Internal):", round(cor_fair_int.underrep$estimate, 3), "p =", round(cor_fair_int.underrep$p.value, 4), "\n")
cat("Correlation (Fairness, External):", round(cor_fair_ext.underrep$estimate, 3), "p =", round(cor_fair_ext.underrep$p.value, 4), "\n")
cat("Correlation (Internal, External):", round(cor_int_ext.underrep$estimate, 3), "p =", round(cor_int_ext.underrep$p.value, 4), "\n\n")

# Building combined outcome model with all mediators
out.fit.combined.underrep <- lm(female_pick ~ treatment + fairness + internal_motivation + external_motivation, data = d_men_pool)

# Run combined mediation analyses
med.out.combined.fairness.underrep <- mediate(med.fit.fairness.underrep, out.fit.combined.underrep, boot = TRUE,
                                     treat = "treatment", boot.ci.type = "perc", mediator = "fairness", sims = 10000)
med.out.combined.internal.underrep <- mediate(med.fit.internal.underrep, out.fit.combined.underrep, boot = TRUE,
                                     treat = "treatment", boot.ci.type = "perc", mediator = "internal_motivation", sims = 10000)
med.out.combined.external.underrep <- mediate(med.fit.external.underrep, out.fit.combined.underrep, boot = TRUE,
                                     treat = "treatment", boot.ci.type = "perc", mediator = "external_motivation", sims = 10000)

# Summarize and print the results for combined analysis
cat("\n--- COMBINED MULTIPLE MEDIATION MODEL RESULTS ---\n\n")
cat("Fairness (controlling for other mediators):\n")
summary(med.out.combined.fairness.underrep)
cat("\nInternal Motivation (controlling for other mediators):\n")
summary(med.out.combined.internal.underrep)
cat("\nExternal Motivation (controlling for other mediators):\n")
summary(med.out.combined.external.underrep)
```
