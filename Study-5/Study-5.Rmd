...
if(USE_API) {
  ...
  qual_data <- fetch_survey(surveyID='SV_diYpq4P39svNYou',
...
r1 <- lm(women_proportion ~ treatment, data=d0)
...
r3 <- lm(women_proportion ~ treatment + gender_code + race_code + age, data=d0)
...
# Age
age.eqn <- list(
  ageeq = as.numeric(age_proportion) ~ women_feedback + age_feedback + location_feedback + university_feedback - 1,
  womeneq = as.numeric(women_proportion) ~ women_feedback + age_feedback + location_feedback + university_feedback - 1
)
...
# Location
location.eqn <- list(
  locationeq = as.numeric(location_proportion) ~ women_feedback + age_feedback + location_feedback + university_feedback - 1,
  womeneq = as.numeric(women_proportion) ~ women_feedback + age_feedback + location_feedback + university_feedback - 1
)
...
# University
university.eqn <- list(
  universityeq = as.numeric(university_proportion) ~ women_feedback + age_feedback + location_feedback + university_feedback - 1,
  womeneq = as.numeric(women_proportion) ~ women_feedback + age_feedback + location_feedback + university_feedback - 1
)
...
age_coef <- lh_result_age$F[2]
location_coef <- lh_result_location$F[2]
university_coef <- lh_result_university$F[2]
...
data.frame(
  `Test` = c("Women Feedback - Age Feedback", "Women Feedback - Location Feedback", "Women Feedback - University Feedback"),
  `Wald Coefficient` = c(age_coef, location_coef, university_coef),
  P_Value = c(p_value_age, p_value_location, p_value_university),
  row.names = 1
)
