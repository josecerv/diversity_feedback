...
if(USE_API) {
  ...
  qual_data <- fetch_survey(surveyID='SV_5aTf8as54qnmh14',
                     label = T,
                     convert = F,
  ...
  d0 <- read.csv('Study2A-replication.csv', check.names = F)
...
if(USE_API) {
  ...
  write.csv(d0, 'Study2A-replication.csv', row.names = FALSE, quote = TRUE)
}
```
...
study2A_replication_estimates <- data.frame(
    Study = "STUDY 2A REPLICATION",
...
saveRDS(study2A_replication_estimates, file = "study2A_replication_estimates.rds")
...
p_poets <- robust_summary(r_poets)$coefficients["poets_shown", "Pr(>|t|)"]
p_books <- robust_summary(r_books)$coefficients["books_shown", "Pr(>|t|)"]
p_oldies <- robust_summary(r_oldies)$coefficients["oldies_shown", "Pr(>|t|)"]
...
sig_poets <- get_sig_stars(p_poets)
sig_books <- get_sig_stars(p_books)
sig_oldies <- get_sig_stars(p_oldies)
...
df_combined <- bind_rows(
  dpoets_plot %>% mutate(Category = "\nWrote Poetry", sig_label = sig_poets),
  dbooks_plot %>% mutate(Category = "\nWrote > 10 books", sig_label = sig_books),
  doldies_plot %>% mutate(Category = "\nWere Born\nin the 1800s", sig_label = sig_oldies),
...
ggsave("Figure-S1-replication.pdf", plot = p_combined, width = 10, height = 8, units = "in", device = cairo_pdf, family = "Times New Roman")
```
