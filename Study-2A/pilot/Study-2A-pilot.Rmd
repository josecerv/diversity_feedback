---
title: "Study 2A Pilot - New Attributes"
...
if(USE_API) {
  ...
  qual_data <- fetch_survey(surveyID='SV_diLrc4ha88321dY',  # New Study ID
                     force_request = T)
} else {
  # Read the processed data directly from CSV
  d0 <- read.csv('Study2A-pilot.csv', check.names = F)
  num_excluded <- unique(d0$num_excluded_total)
}

# Define the NEW categories based on the provided information
women <- c('Zadie Smith', 'Isabel Allende', 'Jane Austen',
           'Joyce Carol Oates', 'Lucy Maud', 'Sylvia Plath')

major_award <- c('Sylvia Plath', 'Ernest Hemingway', 'Gabriel Garcia Marquez',
                 'John Steinbeck', 'Joyce Carol Oates', 'Cormac McCarthy', 'Ray Bradbury')
...
all_three_forms <- c('Charles Dickens', 'Ernest Hemingway', 'George Orwell', 'Herman Melville',
                     'Jack London', 'Joyce Carol Oates', 'Jorge Luis Borges', 'J.R.R. Tolkien',
                     'Nathaniel Hawthorne', 'Sylvia Plath', 'Ray Bradbury', 'Neil Gaiman',
                     'F. Scott Fitzgerald')
...
if(USE_API) {
  ...
  d0 <- qual_data |>
    ...
    mutate(
      # Gender feedback detection remains the same
      gender_feedback = as.numeric(grepl("were women", feedbackItem1) |
                    grepl("were women", feedbackItem2) |
                    grepl("were women", feedbackItem3)),

      # Update attribute feedback detection based on new attributes
      award_shown = as.numeric(grepl("won a Pulitzer Prize, Nobel Prize or National Book Award", feedbackItem1) |
                   grepl("won a Pulitzer Prize, Nobel Prize or National Book Award", feedbackItem2) |
                   grepl("won a Pulitzer Prize, Nobel Prize or National Book Award", feedbackItem3)),
      ...
      # Picks based on new categories
      female_pick = case_when(`choice-7` %in% women ~ 1, TRUE ~ 0),
      award_pick = case_when(`choice-7` %in% major_award ~ 1, TRUE ~ 0),
    ...
    dplyr::select(gender_feedback:forms_pick, base_gender, `choice-1`:`choice-7`,
  ...
  write.csv(d0, 'Study2A-pilot.csv', row.names = FALSE, quote = TRUE)
}
```
...
\hline
\texttt{award\_pick} & Binary indicator of whether a participant selected an author with major awards (Pulitzer, Nobel, National Book Award) for their seventh selection. \\
\hline
...
r2 <- lm(female_pick ~ gender_feedback + classic_shown + award_shown + forms_shown - 1, data=d0)
...
## award feedback
r_award <- lm(award_pick ~ award_shown, data=d0)

# Display the robust_summary with robust standard errors
robust_summary(r_award)
robust_confint(r_award)
...
award_est <- extract_regression_estimates(r_award)
forms_est <- extract_regression_estimates(r_forms)

study2A_pilot_estimates <- data.frame(
    Study = "STUDY 2A PILOT",
    Stimuli = c("gender", "classic", "award", "forms"),
    Estimate = c(gender_est$estimate, classic_est$estimate, award_est$estimate, forms_est$estimate),
    SE = c(gender_est$se, classic_est$se, award_est$se, forms_est$se),
    CI_Lower = c(gender_est$ci_lower, classic_est$ci_lower, award_est$ci_lower, forms_est$ci_lower),
    CI_Upper = c(gender_est$ci_upper, classic_est$ci_upper, award_est$ci_upper, forms_est$ci_upper)
)

# Save the estimates to an RDS file
saveRDS(study2A_pilot_estimates, file = "study2A_pilot_estimates.rds")
...
daward_plot <- d0 |>
  dplyr::select(award_shown, award_pick) |>
  dplyr::group_by(award_shown) |>
  dplyr::summarise(
    n = n(),
    freq = mean(award_pick),
    sd = sd(award_pick) * 100,
    se = (sd(award_pick) / sqrt(n())) * 100
  )|>
  mutate(award_shown = case_when(award_shown==1 ~ "\"Treatment\"",
                          TRUE ~ "\"Control\"")) |>
  rename(Condition = award_shown)
...
p_combined <- ggplot(df_combined, aes(x = Condition, y = freq*100, fill = Condition)) +
  ...
  labs(x = "Feedback on % of authors who...", y = "% of New Authors with the Target Attribute",title = "The Effect of Getting Feedback on Your Author Selections (Pilot)") +
...
ggsave("Figure-S1-pilot.pdf", plot = p_combined, width = 10, height = 8, units = "in", device = cairo_pdf, family = "Times New Roman")
```
...
award.eqn <- list(
  awardeq = as.numeric(award_pick) ~ award_shown + forms_shown + classic_shown + gender_feedback - 1,
  femaleeq = as.numeric(female_pick) ~ award_shown + forms_shown + classic_shown + gender_feedback - 1
)

# Define restriction matrix
restrict_award <- "awardeq_award_shown - femaleeq_gender_feedback = 0"

# Fit the system of equations with the restriction
unrestricted_award <- systemfit(award.eqn, data=d0)

# forms

forms.eqn <- list(
  formseq = as.numeric(forms_pick) ~ award_shown + forms_shown + classic_shown + gender_feedback - 1,
  femaleeq = as.numeric(female_pick) ~ award_shown + forms_shown + classic_shown + gender_feedback - 1
)
...
classic.eqn <- list(
  classiceq = as.numeric(classic_pick) ~ award_shown + forms_shown + classic_shown + gender_feedback - 1,
  femaleeq = as.numeric(female_pick) ~ award_shown + forms_shown + classic_shown + gender_feedback - 1
)
...
lh_result_award = linearHypothesis(unrestricted_award, restrict_award)
...
award_coef <- lh_result_award$F[2]
forms_coef <- lh_result_forms$F[2]
classic_coef <- lh_result_classic$F[2]


# Extracting p-values
p_value_award = lh_result_award$`Pr(>F)`[2]
p_value_forms = lh_result_forms$`Pr(>F)`[2]
p_value_classic = lh_result_classic$`Pr(>F)`[2]

# Include p-values in the summary table
data.frame(
  `Test` = c("Gender Feedback - Award Feedback", "Gender Feedback - Forms Feedback", "Gender Feedback - Classic Feedback"),
  `Wald Coefficient` = c(award_coef, forms_coef, classic_coef),
  P_Value = c(p_value_award, p_value_forms, p_value_classic),
  row.names = 1
)
