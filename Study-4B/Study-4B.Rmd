...
if(USE_API) {
  ...
  qual_data <- fetch_survey(surveyID='SV_8vM8mfohSNgdTxQ',  
                            start_date = "2025-10-06",
                     force_request = T)
} else {
  # Read the processed data directly from CSV
  d0 <- read.csv('Study4B.csv', check.names = F)
...
if(USE_API) {
  ...
  write.csv(d0, 'Study4B.csv', row.names = FALSE, quote = TRUE)
}
```
...
saveRDS(study4B_estimates, file = "../Manuscript_Figures/figure2-estimates/study4B_estimates.rds")
...
p_combined <- ggplot(df_combined, aes(x = Condition, y = freq*100, fill = Condition)) +
  ...
  labs(x = "Feedback on % of authors who...", y = "% of New Authors with the Target Attribute",
       title = "The Effect of Getting Feedback on Your Author Selections") +
  theme(plot.caption = element_text(face = "italic", family = "Times New Roman"),
        ...
        axis.title.x = element_text(face="bold", size = 21, vjust = 17, family = "Times New Roman"),
...
# print(p_combined)
# 
# # Save plot object as RDS for manuscript figure combination
# saveRDS(p_combined, file = "p_combined_study4B.rds")
# 
# # Save the plot
# ggsave("Figure-Study4B.pdf", plot = p_combined, width = 10, height = 8, units = "in", device = cairo_pdf, family = "Times New Roman")
# ggsave("Figure-Study4B.png", plot = p_combined, width = 10, height = 8, units = "in", dpi = 600)
```
