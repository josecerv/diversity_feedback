# Figure S4: Study S1 - Effect of Feedback on Panel Composition
...
# Read data
d0 <- read.csv('../data/StudyS1.csv', check.names = FALSE)
...
# Create plot with styling consistent with manuscript figures
p_combined <- ggplot(df_combined, aes(x = Condition, y = freq * 100, fill = Condition)) +
  ...
  geom_text(data = df_combined %>% filter(Category %in% c('\nFounders') & Condition == '"Treatment"'),
            aes(x = 1.5, y = freq * 100 + se + 5, label = "*"),
            inherit.aes = FALSE, vjust = 0, size = 7, family = "Times New Roman") +
  ...
  labs(x = "Feedback on % of panelists who were...",
       y = "% of New Panelists with the Target Identity") +
  theme(plot.caption = element_text(face = "italic", family = "Times New Roman"),
        ...
        axis.title.x = element_text(face = "bold", size = 18, vjust = 13, family = "Times New Roman"),
...
# Save to PNG (Figure S4) - consistent with manuscript single-panel figures: 10x8, 600 dpi
ggsave("../Figure-S4.png", plot = p_combined, width = 10, height = 8, dpi = 600)

cat("Figure-S4.png saved successfully!\n")
