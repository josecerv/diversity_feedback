...
if(USE_API) {
  ...
  qual_data <- fetch_survey(surveyID='SV_beJTVpRBi2UsN8y',
                     label = T,
                     convert = F,
                     force_request = T)
} else {
  # Read the processed data directly from CSV
  d0 <- read.csv('StudyS1.csv', check.names = F)
...
if(USE_API) {
  d0 <- qual_data |> 
    ...
    dplyr::select(
      list_two, gender_feedback, female, ceo, ceo_pick, founder, founder_pick, tech, tech_pick, base_gender, gender, race, age, gender_code, race_code
    )
  ...
  write.csv(d0, 'StudyS1.csv', row.names = FALSE, quote = TRUE)
}
```
...
```{r echo=TRUE, results='markup'}
# primary model
r1 <- lm(female ~ gender_feedback, data=d0)
...
## which feedback was shown with gender
r2 <- lm(female ~ gender_feedback + ceo + tech + founder, data=d0)
...
r3 <- lm(female ~ gender_feedback + gender_code + race_code + age, data=d0)
...
r4 <- glm(female ~ gender_feedback, family = binomial, data=d0)
...
## founder feedback
r_founder <- lm(founder_pick ~ founder, data=d0)
...
## tech feedback
r_tech <- lm(tech_pick ~ tech, data=d0)
...
```{r echo=TRUE}
## dataframe for Gender information

dgender_plot <- d0 |> 
  dplyr::select(gender_feedback, female) |> 
...
dfounder_plot <- d0 |> 
  dplyr::select(founder, founder_pick) |> 
...
# ggsave("../Supplemental_Figures/Figure-S3.pdf", plot = p_combined, width = 10, height = 8, units = "in", device = cairo_pdf, family = "Times New Roman")
...
founder.eqn <- list(
  foundereq = as.numeric(founder_pick) ~ founder + tech + ceo + gender_feedback - 1,
  femaleeq = as.numeric(female) ~ founder + tech + ceo + gender_feedback - 1
)
...
unrestricted_founder <- systemfit(founder.eqn, data=d0)
#robust_summary(unrestricted_founder)
# Tech

tech.eqn <- list(
  techeq = as.numeric(tech_pick) ~ founder + tech + ceo + gender_feedback - 1,
  femaleeq = as.numeric(female) ~ founder + tech + ceo + gender_feedback - 1
)
...
# Fit the system of equations with the restriction
unrestricted_tech <- systemfit(tech.eqn, data=d0)

# ceo

ceo.eqn <- list(
  ceoeq = as.numeric(ceo_pick) ~ founder + tech + ceo + gender_feedback - 1,
  femaleeq = as.numeric(female) ~ founder + tech + ceo + gender_feedback - 1
)
...
# Fit the system of equations with the restriction
unrestricted_ceo <- systemfit(ceo.eqn, data=d0)
