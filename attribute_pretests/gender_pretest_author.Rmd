---
title: "Gender Attribute Pretest Analysis - Author Attributes"
...
# Survey ID for author gender pretest
survey_id <- "SV_0CIP2gGqtp1GHVI"
...
comparisons <- list(
  c("importance_female", "importance_historical", "Female Author", "Born in 1800s"),
  c("importance_female", "importance_prolific", "Female Author", "Wrote >10 Books"),
  c("importance_female", "importance_poetry", "Female Author", "Wrote Poetry"),
...
# Check if any comparison involving female author is significant
female_comparisons <- t_test_df %>%
  filter(grepl("Female Author", comparison))

if(any(female_comparisons$sig_bonferroni == "Yes")) {
  cat("WARNING: Significant differences found with Female Author attribute\n")
  ...
  cat("✓ No significant differences with Female Author attribute\n")
...
importance_summary <- clean_data %>%
  ...
  mutate(attribute = case_when(
    attribute == "importance_female" ~ "Female Author",
...
female_mean <- importance_summary$Mean[importance_summary$attribute == "Female Author"]
...
cat("2. Female author mean rating: ", female_mean, "\n", sep = "")
...
if(all(female_comparisons$sig_bonferroni == "No")) {
  ...
  cat("The female author attribute is viewed as similarly important\n")
