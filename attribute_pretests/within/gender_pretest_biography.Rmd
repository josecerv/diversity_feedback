---
title: "Gender Attribute Pretest Analysis - Biography Attributes"
...
# Survey ID for gender biography pretest
survey_id <- "SV_eVSIWRL55XQbIiO"
...
# Check what importance columns exist and show sample values
cat("\n=== Checking importance questions ===\n")
importance_cols <- names(survey_data)[grepl("importance", names(survey_data), ignore.case = TRUE)]
if(length(importance_cols) > 0) {
  cat("Found importance columns:", paste(importance_cols, collapse = ", "), "\n\n")

  # Get first non-NA response for each to see the actual questions
  cat("Sample values to identify attributes:\n")
  for(col in importance_cols) {
    non_na <- survey_data[[col]][!is.na(survey_data[[col]])]
    if(length(non_na) > 0) {
      cat(col, "- First value:", as.character(non_na[1]), "\n")
    }
  }
  cat("\nNOTE: Please verify these attributes match your survey design.\n")
  cat("Update the labels below if they differ from what's expected.\n")
}
```
...
clean_data <- survey_data %>%
  ...
  select(ResponseId,
         # Importance ratings for biography attributes
         # NOTE: Update these labels based on your actual survey content
         importance_female = importance_1,  # Female subject (biography about a woman)
         importance_attribute2 = importance_2,  # Placeholder - update with actual attribute
         importance_attribute3 = importance_3,  # Placeholder - update with actual attribute
         importance_attribute4 = importance_4) %>% # Placeholder - update with actual attribute
  ...
  filter(complete.cases(importance_female, importance_attribute2,
                        importance_attribute3, importance_attribute4))
...
# NOTE: Update these labels based on your actual survey attributes
comparisons <- list(
  c("importance_female", "importance_attribute2", "Female Subject", "Attribute 2"),
  c("importance_female", "importance_attribute3", "Female Subject", "Attribute 3"),
  c("importance_female", "importance_attribute4", "Female Subject", "Attribute 4"),
  c("importance_attribute2", "importance_attribute3", "Attribute 2", "Attribute 3"),
  c("importance_attribute2", "importance_attribute4", "Attribute 2", "Attribute 4"),
  c("importance_attribute3", "importance_attribute4", "Attribute 3", "Attribute 4")
)
...
kable(t_test_df %>%
      ...
      caption = "Pairwise T-Tests Between Biography Attributes") %>%
...
# Check if any comparison involving female subject is significant
female_comparisons <- t_test_df %>%
  filter(grepl("Female Subject", comparison))

if(any(female_comparisons$sig_bonferroni == "Yes")) {
  cat("WARNING: Significant differences found with Female Subject attribute\n")
  ...
  cat("✓ No significant differences with Female Subject attribute\n")
...
cat("           ratings across all four biography attributes\n\n")
...
importance_summary <- clean_data %>%
  ...
  mutate(attribute = case_when(
    attribute == "importance_female" ~ "Female Subject",
    attribute == "importance_attribute2" ~ "Attribute 2",  # Update with actual label
    attribute == "importance_attribute3" ~ "Attribute 3",  # Update with actual label
    attribute == "importance_attribute4" ~ "Attribute 4"   # Update with actual label
  )) %>%
  group_by(attribute) %>%
  summarise(
    N = n(),
...
female_mean <- importance_summary$Mean[importance_summary$attribute == "Female Subject"]
...
cat("2. Female subject mean rating: ", female_mean, "\n", sep = "")
...
if(all(female_comparisons$sig_bonferroni == "No")) {
  ...
  cat("differences in perceived importance across the four biography attributes.\n")
  cat("The female subject attribute is viewed as similarly important\n")
  cat("as the other control attributes.\n")
} else {
  ...
  cat("subject attribute should be adjusted or replaced.\n")
}

cat("\n\n*** IMPORTANT ***\n")
cat("Please update the attribute labels in this script to match your actual survey.\n")
cat("Look at the 'Checking importance questions' output above to identify\n")
cat("what each importance_2, importance_3, and importance_4 actually measure.\n")
```
