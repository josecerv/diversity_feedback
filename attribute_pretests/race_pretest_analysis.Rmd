...
cat("importance_1 (minority):", unique(survey_data$importance_1), "\n")
cat("importance_2 (budget):", unique(survey_data$importance_2), "\n")
cat("importance_3 (recent):", unique(survey_data$importance_3), "\n")
cat("importance_4 (runtime):", unique(survey_data$importance_4), "\n\n")
...
comparisons <- list(
  c("importance_minority", "importance_budget", "Minority Protagonist", "Big Budget"),
  c("importance_minority", "importance_recent", "Minority Protagonist", "Recent Release"),
  c("importance_minority", "importance_runtime", "Minority Protagonist", "Long Runtime"),
  c("importance_budget", "importance_recent", "Big Budget", "Recent Release"),
  c("importance_budget", "importance_runtime", "Big Budget", "Long Runtime"),
  c("importance_recent", "importance_runtime", "Recent Release", "Long Runtime")
)
...
# Check if any comparison involving minority protagonist is significant
minority_comparisons <- t_test_df %>%
  filter(grepl("Minority Protagonist", comparison))

if(any(minority_comparisons$sig_bonferroni == "Yes")) {
  cat("WARNING: Significant differences found with Minority Protagonist attribute\n")
  ...
  cat("✓ No significant differences with Minority Protagonist attribute\n")
...
cat("2. Minority protagonist mean rating: ", minority_mean, " (Rank: #", minority_rank, "/4)\n", sep = "")
...
if(ols_p_value < 0.05) {
  ...
  if(nrow(sig_comparisons) > 0) {
    cat("Specific significant differences from Minority Protagonist:\n")
  ...
  all_comparisons <- ols_summary %>%
    filter(term != "Minority Protagonist (Baseline)")
  cat("All comparisons with Minority Protagonist (for reference):\n")
...
if(ols_p_value >= 0.05) {
  ...
  cat("The minority protagonist attribute is viewed as similarly important\n")
  cat("as the other control attributes (budget, recency, runtime).\n")
} else {
...
detail_table <- importance_summary %>%
  ...
  mutate(
    `Difference from Minority` = Mean - minority_mean,
    `% Difference` = round((Mean - minority_mean) / minority_mean * 100, 1)
  )
