---
title: "Race Attribute Pretest Analysis - Biopic Film Attributes"
...
# Survey ID for race biopic pretest
...
importance_summary <- clean_data %>%
  ...
  mutate(attribute = case_when(
    attribute == "importance_minority" ~ "Minority Protagonist",
    attribute == "importance_budget" ~ "Big Budget (>$40M)",
    attribute == "importance_recent" ~ "Released After 2010",
    attribute == "importance_runtime" ~ "Runtime >2 Hours"
  )) %>%
  group_by(attribute) %>%
  summarise(
    N = n(),
...
minority_mean <- importance_summary$Mean[importance_summary$attribute == "Minority Protagonist"]
...
cat("2. Minority protagonist mean rating: ", minority_mean, "\n", sep = "")
