---
title: "Race Attribute Pretest Analysis - Author Attributes"
...
# Survey ID for race author pretest
survey_id <- "SV_5tBTklIN9MdyBqC"
...
clean_data <- survey_data %>%
  ...
  select(ResponseId,
         # Importance ratings (same attributes as gender author survey)
...
comparisons <- list(
  c("importance_minority", "importance_historical", "Minority Author", "Born in 1800s"),
  c("importance_minority", "importance_prolific", "Minority Author", "Wrote >10 Books"),
  c("importance_minority", "importance_poetry", "Minority Author", "Wrote Poetry"),
  c("importance_historical", "importance_prolific", "Born in 1800s", "Wrote >10 Books"),
  c("importance_historical", "importance_poetry", "Born in 1800s", "Wrote Poetry"),
  c("importance_prolific", "importance_poetry", "Wrote >10 Books", "Wrote Poetry")
)
...
kable(t_test_df %>%
      ...
      caption = "Pairwise T-Tests Between Author Attributes") %>%
...
# Check if any comparison involving minority author is significant
minority_comparisons <- t_test_df %>%
  filter(grepl("Minority Author", comparison))

if(any(minority_comparisons$sig_bonferroni == "Yes")) {
  cat("WARNING: Significant differences found with Minority Author attribute\n")
  cat("even after Bonferroni correction:\n")
  sig_minority <- minority_comparisons %>%
    filter(sig_bonferroni == "Yes")
  for(i in 1:nrow(sig_minority)) {
    cat("  -", sig_minority$comparison[i], "(p =", sig_minority$p_bonferroni[i], ")\n")
  }
} else {
  cat("✓ No significant differences with Minority Author attribute\n")
...
cat("           ratings across all four author attributes\n\n")
...
importance_summary <- clean_data %>%
  ...
  mutate(attribute = case_when(
    attribute == "importance_minority" ~ "Minority Author",
    attribute == "importance_historical" ~ "Born in 1800s",
    attribute == "importance_prolific" ~ "Wrote >10 Books",
    attribute == "importance_poetry" ~ "Wrote Poetry"
  )) %>%
  group_by(attribute) %>%
  summarise(
    N = n(),
...
minority_mean <- importance_summary$Mean[importance_summary$attribute == "Minority Author"]
...
cat("2. Minority author mean rating: ", minority_mean, "\n", sep = "")
cat("3. Overall mean across all attributes: ", round(overall_mean, 2), "\n", sep = "")
cat("4. Range of mean ratings: ", round(range_ratings, 2), "\n\n", sep = "")

cat("CONCLUSION:\n")
cat("-----------\n")
if(all(minority_comparisons$sig_bonferroni == "No")) {
  cat("The data support the hypothesis that there are no significant\n")
  cat("differences in perceived importance across the four author attributes.\n")
  cat("The minority author attribute is viewed as similarly important\n")
  cat("as the other control attributes (historical period, prolific output, poetry).\n")
} else {
  cat("The data do not support the hypothesis. There are significant\n")
  cat("differences in perceived importance across the four author attributes.\n")
  cat("Further investigation may be needed to determine if the minority\n")
  cat("author attribute should be adjusted or replaced.\n")
}
```
