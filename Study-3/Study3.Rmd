...
d0 <- qual_data |> 
  ...
  select(female_pick:gender_feedback, founder_comp:male_pool, `choice-1`:`choice-7`, race, gender, age, gender_code, race_code) |> 
...
###########################################################################################
# when reading the csv, use the following command: read.csv('Study4.csv', check.names = F)
###########################################################################################
```
...
p_combined <- ggplot(df_combined, aes(x = Condition, y = freq*100, fill = Condition)) +
  ...
  geom_segment(data = df_combined %>% filter(Category == "Women Underrepresented\nin Panelist Candidate Set" & Condition == "Treatment"), 
    ...
    geom_text(data = df_combined %>% filter(Category == "Women Overrepresented\nin Panelist Candidate Set" & Condition == "Control"), 
                ...
                inherit.aes = FALSE, vjust = 0, family = "Times New Roman", size = 7) +
  theme_bw() +
  scale_fill_manual(values = c("#535350", "#c18354"), labels = c("No gender feedback provided", "Gender feedback provided"), "") +
...
ggsave("Figure-4.pdf", plot = p_combined, width = 10, height = 8, units = "in", device = cairo_pdf, family = "Times New Roman")
