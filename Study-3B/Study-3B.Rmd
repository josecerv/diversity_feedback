...
if(USE_API) {
  d0 <- qual_data |> 
    ...
    dplyr::select(gender_feedback:poli_pick, gender, base_gender, `choice-1`:`choice-7`, race,age, gender_code, race_code) |>
...
saveRDS(study3B_estimates, file = "../Manuscript_Figures/figure2-estimates/study3B_estimates.rds")
...
dpoli_plot <- d0 |>
  dplyr::select(poli_shown, poli_pick) |> 
  group_by(poli_shown) |> 
  dplyr::summarise(
    n = n(),  
    freq = mean(poli_pick),  
...
# # Save the plot
# ggsave("Figure-Study3B.pdf", plot = p_combined_A, width = 10, height = 8, units = "in", device = cairo_pdf, family = "Times New Roman")
# ggsave("Figure-Study3B.png", plot = p_combined_A, width = 10, height = 8, units = "in", dpi = 600)
