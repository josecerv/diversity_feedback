...
if(USE_API) {
  d0 <- qual_data |>
    ...
    # Process motivation and fairness scales
    mutate(
      across(c(I1:E3, fair1:fair3),
             ~ case_when(
    ...
    dplyr::select(female_pick, pool, cond, women_pool, men_pool,
                  ...
                  race, gender, age,
                  internal1Z, internal2Z, internal3Z, internal4Z, internal,
                  external1Z, external2Z, external3Z, external,
...
\hline
\texttt{internal} & Aggregated scale for Internal Motivation to Respond Without Prejudice. \\
\hline
\texttt{external} & Aggregated scale for External Motivation to Respond Without Prejudice. \\
\hline
...
```{r mediation-motivation, echo=FALSE, results='markup'}
...
cat("=======================================================\n")
cat("MEDIATION ANALYSIS: MOTIVATION TO RESPOND WITHOUT PREJUDICE\n")
cat("=======================================================\n\n")
...
med.out.internal <- mediate(med.fit.internal, out.fit.internal, boot = TRUE,
                            ...
                            mediator = "internal", sims = 10000)

# Sensitivity analysis
sens.out.internal <- medsens(med.out.internal, rho.by = 0.01, eps=.01,
...
cat("\n")
plot(sens.out.internal, main = "Sensitivity Analysis: Internal Motivation")
...
med.out.external <- mediate(med.fit.external, out.fit.external, boot = TRUE,
                            treat = "gender_feedback", boot.ci.type = "perc",
                            mediator = "external", sims = 10000)

# Sensitivity analysis
sens.out.external <- medsens(med.out.external, rho.by = 0.01, eps=.01,
                             effect.type = "indirect", sims = 10000)
...
cat("\n")
plot(sens.out.external, main = "Sensitivity Analysis: External Motivation")
...
cat("\n\n--- COMBINED MOTIVATION MODEL (INTERNAL + EXTERNAL) ---\n\n")
...
cat("Combined Model Results:\n\n")
cat("Internal Motivation (controlling for External):\n")
summary(med.out.combined.internal)
cat("\n")
cat("External Motivation (controlling for Internal):\n")
...
cat("--- INTERNAL MOTIVATION (I1-I4) - MEN POOL ---\n\n")
...
# Sobel test
sobel.internal.men <- sobel_test(med.fit.internal.men, out.fit.internal.men, "internal")
...
cat("--- EXTERNAL MOTIVATION (E1-E3) - MEN POOL ---\n\n")
...
# Sobel test
sobel.external.men <- sobel_test(med.fit.external.men, out.fit.external.men, "external")
...
cat("--- INTERNAL MOTIVATION (I1-I4) - WOMEN POOL ---\n\n")
...
# Sobel test
sobel.internal.women <- sobel_test(med.fit.internal.women, out.fit.internal.women, "internal")
...
cat("--- EXTERNAL MOTIVATION (E1-E3) - WOMEN POOL ---\n\n")
...
# Sobel test
sobel.external.women <- sobel_test(med.fit.external.women, out.fit.external.women, "external")
...
```{r moderated-mediation-internal, echo=FALSE, results='markup'}
# Set seed for reproducibility
set.seed(123)

cat("=======================================================\n")
cat("MODERATED MEDIATION: INTERNAL MOTIVATION\n")
cat("Testing if pool composition moderates the mediation effect\n")
cat("=======================================================\n\n")

# Model with interactions to test moderated mediation
# a path: Does pool moderate feedback -> internal?
mod_med_a_internal <- lm(internal ~ gender_feedback * women_pool, data = d0)

# b path: Does pool moderate internal -> outcome (controlling for feedback)?
mod_med_b_internal <- lm(female_pick ~ gender_feedback * women_pool + internal * women_pool, data = d0)

cat("--- Path A: Feedback → Internal (moderated by pool) ---\n")
robust_summary(mod_med_a_internal)
cat("\n\n")

cat("--- Path B: Internal → Outcome (moderated by pool, controlling for feedback) ---\n")
robust_summary(mod_med_b_internal)
cat("\n\n")

# Calculate indirect effects for each pool
# Men pool (women_pool = 0)
a_men <- coef(mod_med_a_internal)["gender_feedback"]
b_men <- coef(mod_med_b_internal)["internal"]
indirect_men <- a_men * b_men

# Women pool (women_pool = 1)
a_women <- coef(mod_med_a_internal)["gender_feedback"] + coef(mod_med_a_internal)["gender_feedback:women_pool"]
b_women <- coef(mod_med_b_internal)["internal"] + coef(mod_med_b_internal)["women_pool:internal"]
indirect_women <- a_women * b_women

# Index of moderated mediation (difference in indirect effects)
index_modmed_internal <- indirect_women - indirect_men
...
cat("\nIndex of Moderated Mediation: ", round(index_modmed_internal, 4), "\n")
cat("(Difference between women pool and men pool indirect effects)\n\n")

# Bootstrap confidence interval for index of moderated mediation
boot_modmed_internal <- function(data, indices) {
  d <- data[indices,]
  m1 <- lm(internal ~ gender_feedback * women_pool, data = d)
  m2 <- lm(female_pick ~ gender_feedback * women_pool + internal * women_pool, data = d)

  a_men <- coef(m1)["gender_feedback"]
  b_men <- coef(m2)["internal"]
  a_women <- coef(m1)["gender_feedback"] + coef(m1)["gender_feedback:women_pool"]
  b_women <- coef(m2)["internal"] + coef(m2)["women_pool:internal"]

  return((a_women * b_women) - (a_men * b_men))
}

library(boot)
boot_results_internal <- boot(data = d0, statistic = boot_modmed_internal, R = 10000)
boot_ci_internal <- boot.ci(boot_results_internal, type = "perc")

cat("Bootstrap 95% CI for Index of Moderated Mediation:\n")
print(boot_ci_internal)
cat("\n")

if (boot_ci_internal$percent[4] > 0 | boot_ci_internal$percent[5] < 0) {
...
```{r moderated-mediation-external, echo=FALSE, results='markup'}
# Set seed for reproducibility
set.seed(123)

cat("=======================================================\n")
cat("MODERATED MEDIATION: EXTERNAL MOTIVATION\n")
cat("Testing if pool composition moderates the mediation effect\n")
cat("=======================================================\n\n")

# Model with interactions
mod_med_a_external <- lm(external ~ gender_feedback * women_pool, data = d0)
mod_med_b_external <- lm(female_pick ~ gender_feedback * women_pool + external * women_pool, data = d0)

cat("--- Path A: Feedback → External (moderated by pool) ---\n")
robust_summary(mod_med_a_external)
cat("\n\n")

cat("--- Path B: External → Outcome (moderated by pool, controlling for feedback) ---\n")
robust_summary(mod_med_b_external)
cat("\n\n")

# Calculate indirect effects
a_men <- coef(mod_med_a_external)["gender_feedback"]
b_men <- coef(mod_med_b_external)["external"]
indirect_men <- a_men * b_men

a_women <- coef(mod_med_a_external)["gender_feedback"] + coef(mod_med_a_external)["gender_feedback:women_pool"]
b_women <- coef(mod_med_b_external)["external"] + coef(mod_med_b_external)["women_pool:external"]
indirect_women <- a_women * b_women

index_modmed_external <- indirect_women - indirect_men

cat("--- INDIRECT EFFECTS ---\n")
cat("Indirect effect (Men Pool, women underrepresented): ", round(indirect_men, 4), "\n")
cat("Indirect effect (Women Pool, women overrepresented): ", round(indirect_women, 4), "\n")
cat("\nIndex of Moderated Mediation: ", round(index_modmed_external, 4), "\n\n")

# Bootstrap CI
boot_modmed_external <- function(data, indices) {
  d <- data[indices,]
  m1 <- lm(external ~ gender_feedback * women_pool, data = d)
  m2 <- lm(female_pick ~ gender_feedback * women_pool + external * women_pool, data = d)

  a_men <- coef(m1)["gender_feedback"]
  b_men <- coef(m2)["external"]
  a_women <- coef(m1)["gender_feedback"] + coef(m1)["gender_feedback:women_pool"]
  b_women <- coef(m2)["external"] + coef(m2)["women_pool:external"]

  return((a_women * b_women) - (a_men * b_men))
}

boot_results_external <- boot(data = d0, statistic = boot_modmed_external, R = 10000)
boot_ci_external <- boot.ci(boot_results_external, type = "perc")

cat("Bootstrap 95% CI for Index of Moderated Mediation:\n")
print(boot_ci_external)
cat("\n")

if (boot_ci_external$percent[4] > 0 | boot_ci_external$percent[5] < 0) {
  cat("INTERPRETATION: The mediation effect DIFFERS significantly between pools.\n")
} else {
  cat("INTERPRETATION: The mediation effect does NOT differ significantly between pools.\n")
  cat("(Consistent with your theory that motivation mediates in both pools)\n")
