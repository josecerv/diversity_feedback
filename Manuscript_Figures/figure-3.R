...
study1A_estimates <- readRDS(paste0(estimate_path, "study1A_estimates.rds"))
study1B_estimates <- readRDS(paste0(estimate_path, "study1B_estimates.rds"))
...
study4_estimates <- readRDS(paste0(estimate_path, "study4_estimates.rds"))

# Combine all estimates into a single data frame
data <- bind_rows(study1A_estimates, study1B_estimates, study2A_estimates, study2B_estimates, study4_estimates)

# Update the study order
study_order <- c('STUDY 1A', 'STUDY 1B', 'STUDY 2A', 'STUDY 2B', 'STUDY 4')
...
ggsave("Figure-3.pdf", combined_plot, width = 16, height = 24, units = "in", device = cairo_pdf, family = "Times New Roman")
