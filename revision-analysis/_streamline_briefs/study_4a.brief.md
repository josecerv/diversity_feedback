# Brief: streamline Study 4A Methods + Results + Mediation
...
# STUDY-SPECIFIC PRESERVATION RULES for Study 4A Methods + Results + Mediation
...
# PRIOR POST-KATY DRAFT for Study 4A Methods + Results + Mediation (calibration anchor; do NOT copy verbatim — use only as a comparison reference)
...
Study 4A tested whether feedback about the race of past selectees increased selection of racial minority authors and whether motivation to respond without prejudice mediated this effect.
...
Procedure. Participants imagined creating a summer fiction reading list for their local library and first selected six authors from a list of 25 real authors, each displayed with a small photograph, name (e.g., James Baldwin, Isabel Allende), most notable work (e.g., Go Tell It on the Mountain, The House of the Spirits), and count of total published works (e.g., authored 6 books and several short stories and poems; authored 6 books). Eight of the 25 authors (32%) were racial minorities. Participants were randomly assigned to a no race feedback condition, in which they received summary statistics about the percentage of selected authors who were born in the 1800s, wrote more than 10 books, and/or wrote poetry, or to a race feedback condition, in which they always received feedback about the percentage of selected authors who were racial minorities alongside one or two randomly drawn comparison feedback statistics. They then selected a seventh and final author from the original author candidate set, where their prior selections were highlighted. Afterward, participants completed adapted Plant and Devine (1998) internal motivation to respond without prejudice items (four items; e.g., “Given my personal values and beliefs, an important factor in my selection of the seventh author to add to my list was my desire to promote the success of racial minorities.”; Cronbach’s α = 0.93) and external motivation to respond without prejudice items (three items; e.g., “When making my decision about who to select as my seventh author, I considered the social pressures I face to appear to promote racial minorities where they are typically underrepresented.”; Cronbach’s α = 0.90), all on 1 (Strongly disagree) to 7 (Strongly agree) scales; as pre-registered, items were z-scored and averaged within each scale. See Materials Section: Study 4A in the Appendix for complete study materials.

Results

Initial selections featured racial minority authors 21.8% of the time, with balance across conditions (20.7% in the no race feedback condition and 22.9% in the race feedback condition, t(993.88) = -1.58, p = .114, 95% CI = [-0.049, 0.005]; see Appendix Table S7). Following our pre-registration, we ran an OLS regression with robust standard errors predicting whether the seventh and final author was a racial minority from assignment to the race feedback condition. Participants were significantly more likely to choose a racial minority author in the race feedback condition (41.60% chose a racial minority author) than in the no race feedback condition (28.20% chose a racial minority author; B = 0.134, 95% CI = [0.075, 0.193], t(998) = 4.48, p < .001; see Table S7, Model 1). The effect of assignment to the race feedback condition on selection of a racial minority author was significantly larger than the effect of assignment to receive descriptive feedback about the percentage of initial authors who (1) wrote poetry (F(1, 3984) = 6.19, p = .013), (2) wrote more than 10 books (F(1, 3984) = 5.50, p = .019), or (3) were born in the 1800s (F(1, 3984) = 5.26, p = .022) on subsequent selections of final authors who, respectively, (1) wrote poetry, (2) wrote more than 10 books, or (3) were born in the 1800s (see Table S7, Models 5-8).

Mediation. Following our pre-registration, we tested each proposed mediator independently using a 10,000-sample bootstrapped mediation model and a Sobel test. The 95% bias-corrected CI for the indirect effect of internal motivation to respond without prejudice excluded zero (indirect effect = 0.06, 95% CI = [0.034, 0.09]), suggesting that internal motivation helped account for the effect of assignment to the race feedback condition on selecting a racial minority final author. Using Imai et al.’s (2010) average causal mediation effect approach, 44.7% of the effect occurred through mediation by internal motivation to respond without prejudice. The 95% bias-corrected CI for the indirect effect of external motivation to respond without prejudice also excluded zero (indirect effect = 0.019, 95% CI = [0.005, 0.03]), suggesting that external motivation also helped account for the effect of race feedback (vs. no race feedback) on selecting a racial minority final author; Imai et al.’s average causal mediation effect approach suggested that 14.1% of the effect occurred through external motivation to respond without prejudice. Given these results, and following our pre-registration, we tested both mediators in a multiple mediation model that addressed potential multicollinearity between the correlated measures (r = 0.72, p < .001). Once accounting for the correlation between constructs, internal motivation to respond without prejudice significantly mediated the effect (indirect effect = 0.08, 95% CI = [0.042, 0.11]), whereas external motivation to respond without prejudice did not (indirect effect = -0.014, 95% CI = [-0.03, 0.00]).

## Word count: 1237 -> 829 (saved 408)

## Preservation notes
- Cut the broad Study 4 framing and excluded Study 4B/Discussion from AFTER per the Study 4A-only brief.
- Kept demographics per Katy red line #1 and removed repeated eligibility boilerplate covered by the Study 1 footnote.
- Compressed the procedure while preserving stimulus details, the 25-author set, the 8 racial-minority authors/32% baseline, condition structure, and “where their prior selections were highlighted” per Katy red line #4.
- Kept internal/external MRWP measure detail, item examples, z-scoring/averaging, and Cronbach’s α values because mediation is the Study 4 contribution.
- Kept the balance check with p = .114 and an appendix pointer per Katy red line #9.
- Kept cell means and primary OLS statistics with “chose a racial minority author” phrasing per Katy red line #5.
- Kept verbose primary Wald F-tests and all F-statistics per Katy red line #6.
- Preserved mediation detail: bootstrapped/Sobel tests, indirect effects/CIs, ACME percentages, multiple-mediation specification, correlation, and multicollinearity rationale.
- Departed from the robustness-one-liner template because the SOURCE does not report Study 4A demographic/logistic robustness results; kept Table S7 pointers instead of inventing a claim.
