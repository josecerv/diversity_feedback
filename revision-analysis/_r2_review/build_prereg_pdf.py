"""Pre-registered NPR Attribute Pretest analysis + PDF (N=300).

New survey: SV_aeIi4xABcJiz1vU (https://wharton.qualtrics.com/jfe/form/SV_aeIi4xABcJiz1vU)

Mirrors build_option1_pdf.py exactly so results are comparable to the prior
exploratory pretest (Survey1_NPR_Attribute_Test.pdf). Four-row table:
  Attribute | N | M | SD | Cohen's d | p (vs. gender)
Welch two-sample test against the "were women." reference row.
"""
...
CSV  = ROOT / "prereg_SV_aeIi4xABcJiz1vU.csv"
OUT  = ROOT / "Survey1_NPR_Attribute_Test_PREREG.pdf"
...
def main():
    ...
    print(f"{'attribute':55s} {'N':>4} {'M':>6} {'SD':>6} {'t':>7} {'df':>7} {'p':>9} {'d':>7}")
    print(f"{'were women. (ref)':55s} {len(gender):>4} {np.mean(gender):>6.3f} {np.std(gender, ddof=1):>6.3f}")
    for c in COMPARISONS:
        y = df.loc[df["attribute"] == c, "importance_num"].to_numpy()
        t, dfree, p, d = welch(gender, y)
        print(f"{c:55s} {len(y):>4} {np.mean(y):>6.3f} {np.std(y, ddof=1):>6.3f} {t:>7.3f} {dfree:>7.2f} {p:>9.4f} {d:>+7.3f}")
        rows.append({
    ...
    title_l2 = "Survey 1 NPR Attribute Test Between Subjects (pre-registered, N=300)"
    subtitle = (
